...
func (s *t_app_state) initialize_client(caddr string) error {
	...
	c.peer.On("greeting", func(p *p2p.Packet) {
		...
		if is_host == 1 {
			...
			s.chunks = make([]Chunk, s.max_chunks_count)
		}
	...
	c.peer.On("hereyago", func(packet *p2p.Packet) {
		...
		if len(s.chunk_segs_to_recieve) == 0 { // recieved all segments for the current chunk
			...
			packet := p2p.NewPacket()
			packet.WriteUint32(uint32(len(s.chunks)))
			s.broadcast("lookatme", packet) 
			s.request_next_chunk_segs()
		}
	})
...
func (s *t_app_state) request_next_chunk_segs() error {
	...
	targets := make(map[string]*t_client) // TODO: targets need to be pre-computed
	...
	i := 0
	size := DataChunkSize / len(targets)
	for _, t := range targets {
		packet := p2p.NewPacket()

		packet.WriteUint32(curr_chunk_index)
		packet.WriteUint32(uint32(size * i)) // it's 1 based
		packet.WriteUint32(uint32(size * (i + 1)))

		t.peer.Send("gimme", packet)
